...
export const banishSources: BanishSource[] = [
	...
	},
	{
		name: "Middle Finger",
		available: () =>
			!get("_mafiaMiddleFingerRingUsed") && have($item`mafia middle finger ring`),
...
export const runawaySources: RunawaySource[] = [
	...
	},
	{
		...
		prepare: (): void => {
			...
			if (
				floor((familiarWeight(runawayFamiliar) + weightAdjustment()) / 5) <=
				get("_banderRunaways")
			) {
	...
	},
	{
		...
		prepare: () => {
			...
			if (itemAmount($item`peppermint sprout`) >= 5) {
				...
				5 * mallPrice($item`peppermint sprout`)
			) {
