	...
	tasks: [
		{
			...
			do: (): void => {
				...
				if (!get("_mimeArmyShotglassUsed") && have($item`mime army shotglass`)) {
					const shotglassDiet = Diet.plan(MPA, shotglassMenu(), {
						food: 0,
						booze: 1,
						spleen: 0,
					});
		...
		},
		{
			...
			do: (): void => {
				...
				if (
					...
					familiarEquippedEquipment($familiar`Left-Hand Man`) ===
...
function acquire(qty: number, item: Item, maxPrice?: number, throwOnFail = true): number {
	...
	if (
		...
		print(
			`Trying to acquire ${qty} ${item.plural}; max price ${maxPrice.toFixed(0)}.`,
			"green"
		);
	...
	return cookBookBatFoodAvailable.map(
		(food) =>
			new MenuItem(food, {
				...
				maximum: $items`Calzone of Legend, Pizza of Legend, Deep Dish of Legend`.includes(
					food
				)
					? 1
					: 99,
...
function consumeDiet(diet: Diet<MenuData>, mpa: number) {
	...
	while (sumNumbers(plannedDietEntries.map((e) => e.quantity)) > 0) {
		...
		for (const dietEntry of plannedDietEntries) {
			...
			if (organ === "food") {
				quantity = clamp(
					Math.floor(getRemainingStomach() / dietEntry.target().size),
					0,
					quantity
				);
			} else if (organ === "booze") {
				quantity = clamp(
					Math.floor(getRemainingLiver() / dietEntry.target().size),
					0,
					quantity
				);
				if (
				...
				quantity = clamp(
					Math.floor(getRemainingSpleen() / dietEntry.target().size),
					0,
					quantity
				);
			}
