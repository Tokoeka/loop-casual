...
const Diary: Task[] = [
	{
		...
		outfit: {
			equip: !get("candyCaneSwordBlackForest")
				? $items`candy cane sword cane, blackberry galoshes`
				: $items`blackberry galoshes`,
			familiar: $familiar`Reassembled Blackbird`,
			modifier: "+combat",
		},
		choices: {
			923:
				haveEquipped($item`candy cane sword cane`) && !get("candyCaneSwordBlackForest")
					? 5
					: 1,
			924: 1,
		},
	...
	},
	{
		...
		do: (): void => {
			...
			visitUrl(
				"shop.php?whichshop=blackmarket&action=buyitem&whichrow=281&ajax=1&quantity=1"
			);
		},
		limit: { tries: 1 },
		freeaction: true,
...
const Desert: Task[] = [
	{
		...
		completed: () =>
			have($item`Shore Inc. Ship Trip Scrip`) || have($item`UV-resistant compass`),
		do: $location`The Shore, Inc. Travel Agency`,
		choices: { 793: 1 },
		limit: { tries: 1 },
		freeaction: true,
...
const Pyramid: Task[] = [
	...
	},
	{
		...
		completed: () =>
			have($item`ancient bronze token`) ||
			have($item`ancient bomb`) ||
			get("pyramidBombUsed"),
		do: () => rotatePyramid(4),
		limit: { tries: 1 },
	},
