import { cliExecute, haveEquipped, myHash, use, visitUrl } from "kolmafia";
...
const Apartment: Task[] = [
	...
	},
	{
		...
		acquire: [
			{
				item: $item`short writ of habeas corpus`,
				num: 1,
				price: runawayValue,
				optional: true,
			},
...
const Bowling: Task[] = [
	...
	},
	{
		...
		choices: {
			788:
				haveEquipped($item`candy cane sword cane`) && !get("candyCaneSwordBowlingAlley")
					? 2
					: 1,
		},
		limit: { tries: 5 },
		outfit: {
			equip: !get("candyCaneSwordBowlingAlley")
				? $items`candy cane sword cane`
				: $items`none`,
		},
	},
	{
