...
export const LevelingQuest: Quest = {
	...
	tasks: [
		...
		},
		{
			...
			do: () =>
				cliExecute(
					`bastille ${myPrimestat() === $stat`Mysticality` ? "myst" : myPrimestat()}`
				),
			limit: { tries: 1 },
			freeaction: true,
			outfit: {
		...
		},
		{
			name: "Glitch",
			after: [],
			ready: () => have($item`[glitch season reward name]`) && !get("_glitchMonsterFights"),
			completed: () => get("_glitchMonsterFights") === 1,
			prepare: () => {
				restoreHp(myMaxhp());
			...
			combat: new CombatStrategy()
				.macro(
					new Macro()
						.trySkill($skill`Curse of Marinara`)
			...
			outfit: {
				...
				equip: $items`June cleaver, makeshift garbage shirt, Greatest American Pants, familiar scrapbook`,
				modifier: "mainstat, 4exp, HP",
			},
			effects: $effects`Ruthlessly Efficient, Mathematically Precise, Blood Bubble`,
			limit: { tries: 1 },
			freecombat: true,
		},
		{
		...
		},
		{
			...
			completed: () =>
				get("_sausageFights") > 0 || myLevel() >= args.levelto || !args.professor,
			...
			combat: new CombatStrategy()
				.macro((): Macro => {
					if (
						...
						get("_neverendingPartyFreeTurns") >= 6 &&
